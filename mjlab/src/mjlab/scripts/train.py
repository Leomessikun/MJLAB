...
def run_train(task: str, cfg: TrainConfig) -> None:
  ...
  if isinstance(cfg.env, TrackingEnvCfg):
    # Prefer WandB artifact if provided, otherwise allow direct local file path override.
    ...
    elif getattr(cfg.env.commands.motion, "motion_file", ""):
      # motion_file already provided via CLI override: --env.commands.motion.motion_file /path/to/motion.npz
      registry_name = None
      print(f"[INFO] Using local motion file: {cfg.env.commands.motion.motion_file}")
    else:
      raise ValueError(
        "Must provide either --registry-name or --env.commands.motion.motion_file for tracking tasks."
      )
